...
resource "aws_vpc_peering_connection" "default_main_vpcs" {
...
resource "aws_route" "default_vpc_rt" {
  ...
  vpc_peering_connection_id = aws_vpc_peering_connection.default_main_vpcs.id
  depends_on                = [aws_vpc_peering_connection.default_main_vpcs]
}

resource "aws_route" "main_vpc_rt" {
  route_table_id            = "rtb-098e83b5050bca1e6"
